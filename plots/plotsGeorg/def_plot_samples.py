...
#Helper Box in plots
def drawObjects(isTrainData, Flavour, Samples, ptCut, RelIsoCut):
    tex = ROOT.TLatex()
    tex.SetNDC()
    ...
    lines = [
      ...
      (1.03, 0.97, ('Electron ' if Flavour=="Ele" else 'Muon ')+('TrainData' if isTrainData else 'TestData')+' ('+Samples+')'+' , '+ptCut+('' if RelIsoCut=='' else ', relIso<='+str(RelIsoCut))),
    ]
    return [tex.DrawLatex(*l) for l in lines]

def drawObjectsSmall(isTrainData, Flavour, Samples, ptCut, RelIsoCut):
    tex = ROOT.TLatex()
    tex.SetNDC()
