...
selections=["(abs(lep_pdgId)==11 && lep_isPromptId==1)","(abs(lep_pdgId)==11 && lep_isNonPromptId==1)","(abs(lep_pdgId)==11 && lep_isFakeId==1)"]
plotnames=["actual_Prompt_electrons","actual_NonPrompt_electrons","actual_Fake_electrons"]
i=0
...
for selection in selections:
    ...
    selectionString = selection # could be a complicated cut
    ...
    plots = []
    # Add plots
    plots.append(Plot(name=plotname+'_prob_Prompt',
        texX = 'prob_isPrompt', texY = 'Number of Events',
        attribute = lambda lepton, sample: lepton.prob_lep_isPromptId,
        binning=[30,0,1],
    ))
    
    plots.append(Plot(name=plotname+'_prob_NonPrompt',
        texX = 'prob_isNonPrompt', texY = 'Number of Events',
        attribute = lambda lepton, sample: lepton.prob_lep_isNonPromptId,
        binning=[30,0,1],
    ))

    plots.append(Plot(name=plotname+'_prob_Fake',
        texX = 'prob_isFake', texY = 'Number of Events',
...
#    ))
    ...
    def drawPlots(plots, dataMCScale):
      for log in [False, True]:
        plot_directory_ = os.path.join(plot_directory, 'histogram_plots', 'discriminator_shapes', plotname, ("log" if log else "lin"))
        for plot in plots:
