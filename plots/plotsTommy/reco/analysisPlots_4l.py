...
def nonZ1_l( event, sample ):
    event.nonZ1_l_ee   = abs(event.lep_pdgId[event.nonZ1_l1_index_4l]) == 11 and abs(event.lep_pdgId[event.nonZ1_l2_index_4l]) == 11
    event.nonZ1_l_mumu = abs(event.lep_pdgId[event.nonZ1_l1_index_4l]) == 13 and abs(event.lep_pdgId[event.nonZ1_l2_index_4l]) == 13
    event.nonZ1_l_OF   = abs(event.lep_pdgId[event.nonZ1_l1_index_4l]) != abs(event.lep_pdgId[event.nonZ1_l2_index_4l])
    event.nonZ1_l_SF   = event.nonZ1_l_ee or event.nonZ1_l_mumu   
...
def getCond(*args):
    def _condition(event, sample):
        return all([getattr( event, arg) for arg in args])
    return _condition
...
#    ))
    ...
    plots.append(Plot(
        name = "totalLeptonCharge", texX = 'event.totalLeptonCharge', texY = 'Number of Events',
        attribute = lambda event, sample:event.totalLeptonCharge,
        binning=[8,-4,4],
    ))    
